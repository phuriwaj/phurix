/**
 * HeroSection — marketing hero with auto-playing video background, infinite
 * logo slider, and scroll-aware nav. Copy-paste port from a shadcn-style
 * block, adapted for Astro:
 *   - `next/link` is replaced with plain `<a>` (Astro view-transitions will
 *     intercept same-origin `<a>` clicks via `<ClientRouter />`).
 *   - The Radix `Slot` in `Button asChild` works with `<a>` directly.
 *
 * Tokens used: shadcn semantic palette (added in Phase 2 of the migration).
 *
 * Cite: DESIGN.md §7 (Components — `hero-section-5`).
 */
